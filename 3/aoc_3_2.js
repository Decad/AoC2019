...
function walkToIntercect(point, path) {
    let walked = 0;
    let location = [0, 0];
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        switch (direction) {
            case 'R':
                if (point[1] == location[1] && (point[0] - location[0] <= length)) {
                    return walked + (point[0] - location[0]);
                } else {
                    location = [location[0] + length, location[1]];
                }
                break;
            case 'L':
                if (point[1] == location[1] && (location[0] - point[0] <= length)) {
                    return walked + (location[0] - point[0]);
                } else {
                    location = [location[0] - length, location[1]];
                }
                break;
            case 'D':
                if (point[0] == location[0] && (location[1] - point[1] <= length)) {
                    return walked + (location[1] - point[1]);
                } else {
                    location = [location[0], location[1] - length];
                }
                break;
            case 'U':
                if (point[0] == location[0] && (point[1] - location[1] <= length)) {
                    return walked + (point[1] - location[1]);
                } else {
                    location = [location[0], location[1] + length];
                }
                break;
...
function solve() {
    ...
    const closest = intercections.reduce((current, point) => {
        const dist = walkToIntercect(point, path1) + walkToIntercect(point, path2);
        if (dist < current)
            return dist;
